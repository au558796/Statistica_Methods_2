...
Created on Mon Oct 10 16:22:02 2016
...
columns = ['subject', 'age', 'gender', 'trial', 'colour1', 'colour2', 'col1_name', 'col2_name', 'colour_distance', 'category', 'accuracy', 'rt']
index = np.arange(0)
...
the "left" or "right" error key) as quickly as possible, while being also as accurate
...
end_matching = unicode("""
This is the end of the colour matching task. Now, we would like you to label
the colours for us, an then you are done. For each colour, select the colour 
term that you would (rather) use to describe this colour (using the error keys).
...
for rep in range(2): #repeat 2 times
    ...
    for colour_pair in colour_pairs:
        ...
        DATA = DATA.append({'subject': V['subject_ID'], 'age': V['age'], 'gender': V['gender'], 'trial': trial, 'colour1':str(colour_pair[0]), 'colour2':str(colour_pair[1]), 'colour_distance': outcome[2], 'accuracy': outcome[0], 'rt': outcome[1]}, ignore_index=True)
