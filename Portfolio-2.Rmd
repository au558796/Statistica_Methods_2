...
Expand on the MLU Python script to test for the second hypothesis that vocabulary size (the number of unique words) used by mothers are correlated with children's vocabularies through different ages (i.e. through the transcripts). In Python, you can use the command ```set()``` to get the unique words from a text/transscript. If you choose to also do this analysis, it should be reported in ways similar to the previous one.
