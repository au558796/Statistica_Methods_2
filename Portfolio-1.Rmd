...
plot1<- ggplot(newdata , aes (x = Ocular_dominance , y = Hold_breath, fill=Ocular_dominance)) + 
...
# Plot comparing reaction time 
plot2<- ggplot(data,aes(Ballon_rt)) +
  geom_histogram(aes(y=..density..),colour="Black",fill="White", binwidth = 5) +
...
plot3<- ggplot(data, aes (x = Shoe_size, y = Hold_breath), fill=) +
  geom_point(color="Black") +
  labs (x = "European Shoe Size", y = "Time in Seconds", 
        title = "Shoe Size Vs. Hold Breath Length") +
  theme_classic() +
...
plot4<- ggplot(data, aes(Shoe_size,Hold_breath, colour = Gender)) +
  geom_point() +
  geom_smooth(method='lm') +
  labs(title = "relation between shoe size and holding breath divided by gender",
       x = "Shoe size", "Holding breath (secs)")
plot4
